...
本仓库是在原 Solara 基础上的自定义改造版本，重点增强了主题视觉、移动端适配、列表管理、网页歌单转换器、多功能添加。
...
（以下在支持透明主题的浏览器下效果最佳）
- 固定透明主题配色：不再根据封面动态改变整体配色，减少歌曲切换时的视觉跳变。
- 浅色模式重做：整体调整为更克制的白灰色体系。
- 深色模式重做：浅色模式的反版，提升层次与可读性。

### 播放与列表增强

- 列表排序：收藏列表tab和歌单tab加入按时间升序 / 降序排序。
- 拖拽歌曲排序：播放列表、收藏列表和自定义歌单中的歌曲支持拖拽调整顺序，便于手动整理播放队列和歌单内容。
- tab 内搜索：播放列表、收藏列表和自定义歌单均加入独立搜索框，可在当前 tab 内按歌曲名、歌手名或专辑名快速筛选歌曲。
- 歌单歌曲导入：自定义歌单页面也加入了导入歌曲功能，可将外部 JSON 导入当前歌单。
...
- 网页歌单转换器：新增 `/converter.html`，可直接使用。
- 未匹配报告：转换失败或未匹配的歌曲会写入 not-found 报告，便于后续手动补充。
- 本地转换脚本：（该条可以忽略，歌单提取请用网页版本）
- QQ 本地转换限制说明：（该条可以忽略，歌单提取请用网页版本）
...
- QQ 音乐歌单不会直接输出腾讯源，会先读取歌名和歌手，再匹配酷我源歌曲，最终输出格式为 `source: "kuwo"`。
- 无法匹配的歌曲会进入未匹配报告。
...
如果必须使用本地脚本，可尝试传入 QQ 音乐网页 Cookie：(或发给ai代码和歌单链接叫他帮忙提取
