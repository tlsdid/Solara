const dom = {
    ...
    customPlaylistSearchInput: document.getElementById("customPlaylistSearchInput"),
    importCustomPlaylistBtn: document.getElementById("importCustomPlaylistBtn"),
    importCustomPlaylistInput: document.getElementById("importCustomPlaylistInput"),
    createCustomPlaylistBtn: document.getElementById("createCustomPlaylistBtn"),
    renameCustomPlaylistBtn: document.getElementById("renameCustomPlaylistBtn"),
    sortCustomPlaylistBtn: document.getElementById("sortCustomPlaylistBtn"),
    deleteCustomPlaylistBtn: document.getElementById("deleteCustomPlaylistBtn"),
    ...
    importToCustomPlaylist: document.getElementById("importToCustomPlaylist"),
    ...
    mobileImportCustomPlaylistBtn: document.getElementById("mobileImportCustomPlaylistBtn"),
    ...
    mobileSortCustomPlaylistBtn: document.getElementById("mobileSortCustomPlaylistBtn"),
    ...
    mobileSortFavoritesBtn: document.getElementById("mobileSortFavoritesBtn"),
...
const state = {
    ...
    playlistFilter: "",
    favoritesFilter: "",
    customPlaylistFilter: "",
    favoriteSortOrder: "desc",
    customPlaylistSortOrder: "desc",
    dragSort: null,
...
state.favoriteSortOrder = inferAddedAtOrder(state.favoriteSongs);
...
function setupInteractions() {
    ...
    function initializeCustomPlaylistEventHandlers() {
        ...
        if (dom.mobileSortCustomPlaylistBtn) {
            dom.mobileSortCustomPlaylistBtn.addEventListener("click", (event) => {
                event.preventDefault();
                sortCustomPlaylistByAddedAt();
            });
        }
        ...
        if (dom.sortCustomPlaylistBtn) {
            dom.sortCustomPlaylistBtn.addEventListener("click", (event) => {
                event.preventDefault();
                sortCustomPlaylistByAddedAt();
            });
        }
    ...
    if (dom.importCustomPlaylistBtn && dom.importCustomPlaylistInput) {
        dom.importCustomPlaylistBtn.addEventListener("click", () => {
            dom.importCustomPlaylistInput.value = "";
            dom.importCustomPlaylistInput.click();
        });
        dom.importCustomPlaylistInput.addEventListener("change", handleImportCustomPlaylistChange);
    }

    if (dom.mobileImportCustomPlaylistBtn && dom.importCustomPlaylistInput) {
        dom.mobileImportCustomPlaylistBtn.addEventListener("click", () => {
            dom.importCustomPlaylistInput.value = "";
            dom.importCustomPlaylistInput.click();
        });
    }
    ...
    if (dom.sortFavoritesBtn) {
        dom.sortFavoritesBtn.addEventListener("click", sortFavoritesByAddedAt);
    }
    ...
    if (dom.mobileSortFavoritesBtn) {
        dom.mobileSortFavoritesBtn.addEventListener("click", sortFavoritesByAddedAt);
    }
    ...
    if (dom.importToCustomPlaylist) {
        dom.importToCustomPlaylist.addEventListener("click", (event) => {
            event.preventDefault();
            closeImportSelectedMenu();
            importSelectedSearchResults("custom");
        });
    }
...
function importSelectedSearchResults(target = "playlist") {
    ...
    if (target === "custom") {
        const playlist = getSelectedCustomPlaylist() || promptCreateCustomPlaylist();
        if (!playlist) {
            showNotification("请先创建或选择歌单", "warning");
            return;
        }
        ...
        songsToAdd.forEach((song) => {
            assignAddedAt(song, importedAt - added);
        ...
        if (added > 0) {
            ...
            showNotification(`成功导入 ${added} 首歌曲到歌单${duplicateHint}`, "success");
        } else {
            showNotification("选中的歌曲已在歌单中", "warning");
        }
        return;
    }
...
function handleImportedCustomPlaylistItems(rawItems) {
    const playlist = getSelectedCustomPlaylist() || promptCreateCustomPlaylist();
    if (!playlist) {
...
function handleImportCustomPlaylistChange(event) {
    const input = event?.target;
    const file = input?.files?.[0];
    if (!file) {
        return;
    ...
    reader.onload = () => {
        try {
            ...
            const { added, duplicates } = handleImportedCustomPlaylistItems(items);
            if (added > 0) {
                const duplicateHint = duplicates > 0 ? `，${duplicates} 首已存在` : "";
                showNotification(`成功导入 ${added} 首歌曲到歌单${duplicateHint}`, "success");
            } else {
                showNotification("文件中的歌曲已在歌单中", "warning");
            }
        } catch (error) {
            console.error("导入歌单歌曲失败:", error);
            showNotification("导入失败，请确认文件格式", "error");
        } finally {
            if (input) {
                input.value = "";
    ...
    reader.onerror = () => {
        console.error("读取歌单文件失败:", reader.error);
        showNotification("无法读取歌单文件", "error");
        if (input) {
            input.value = "";
        }
    };
...
function getSongSearchText(song) {
    ...
    const artist = Array.isArray(song.artist)
        ? song.artist.join(" ")
        : String(song.artist || "");
    return `${song.name || ""} ${artist} ${song.album || ""}`.toLowerCase();
}

function getFilteredSongEntries(items, filterText) {
    const normalizedFilter = normalizeLibrarySearchText(filterText);
    const sourceItems = Array.isArray(items) ? items : [];
    return sourceItems
        .map((song, index) => ({ song, index }))
        .filter(({ song }) => !normalizedFilter || getSongSearchText(song).includes(normalizedFilter));
}
...
function assignAddedAt(song, timestamp = Date.now()) {
    if (song && typeof song === "object" && normalizeAddedAt(song.addedAt) === null) {
        song.addedAt = timestamp;
    }
    return song;
}

function ensureAddedAtForItems(items) {
    if (!Array.isArray(items)) {
        return;
    }
    const base = Date.now();
    items.forEach((song, index) => {
        assignAddedAt(song, base - index);
    });
}

function updateSortButtonIcons(buttons, order) {
    const iconClass = order === "asc" ? "fa-arrow-up-wide-short" : "fa-arrow-down-wide-short";
    buttons.filter(Boolean).forEach((button) => {
        const label = order === "asc" ? "按加入时间正序" : "按加入时间倒序";
        button.setAttribute("title", label);
        button.setAttribute("aria-label", label);
...
function sortSongsByAddedAt(items, order) {
    if (!Array.isArray(items) || items.length < 2) {
        return;
    }
    ensureAddedAtForItems(items);
    const direction = order === "asc" ? 1 : -1;
    items.sort((a, b) => {
        const aTime = normalizeAddedAt(a?.addedAt) || 0;
        const bTime = normalizeAddedAt(b?.addedAt) || 0;
        return (aTime - bTime) * direction;
    });
}

function inferAddedAtOrder(items) {
    if (!Array.isArray(items) || items.length < 2) {
        return "desc";
    }
    const first = normalizeAddedAt(items[0]?.addedAt) || 0;
    const last = normalizeAddedAt(items[items.length - 1]?.addedAt) || 0;
...
function createDragHandle(list, index) {
    return `<button class="playlist-item-drag" type="button" draggable="true" data-drag-list="${list}" data-index="${index}" title="拖拽排序" aria-label="拖拽排序"><i class="fas fa-grip-lines"></i></button>`;
}

function remapMovedIndex(currentIndex, fromIndex, toIndex) {
...
function getSortableLibraryItems(list) {
    if (list === "playlist") {
        return state.playlistSongs;
    }
    if (list === "favorites") {
        return ensureFavoriteSongsArray();
    }
    if (list === "custom") {
        const playlist = getSelectedCustomPlaylist();
...
function persistLibrarySort(list, fromIndex, toIndex) {
    if (list === "playlist") {
        state.currentTrackIndex = remapMovedIndex(state.currentTrackIndex, fromIndex, toIndex);
        savePlayerState();
        renderPlaylist();
        return;
    }
    if (list === "favorites") {
        state.currentFavoriteIndex = remapMovedIndex(state.currentFavoriteIndex, fromIndex, toIndex);
        saveFavoriteState();
        renderFavorites();
        return;
    }
    if (list === "custom") {
        state.currentCustomTrackIndex = remapMovedIndex(state.currentCustomTrackIndex, fromIndex, toIndex);
        saveCustomPlaylistState();
        renderCustomPlaylists();
    }
}

function sortFavoritesByAddedAt() {
    const favorites = ensureFavoriteSongsArray();
    if (favorites.length === 0) {
        return;
    }
    const currentKey = getSongKey(favorites[state.currentFavoriteIndex]);
    state.favoriteSortOrder = state.favoriteSortOrder === "desc" ? "asc" : "desc";
    sortSongsByAddedAt(favorites, state.favoriteSortOrder);
    if (currentKey) {
        const nextIndex = favorites.findIndex((song) => getSongKey(song) === currentKey);
        if (nextIndex >= 0) {
...
function sortCustomPlaylistByAddedAt() {
    const playlist = getSelectedCustomPlaylist();
    const items = Array.isArray(playlist?.items) ? playlist.items : [];
    if (items.length === 0) {
        return;
    }
    const currentKey = getSongKey(items[state.currentCustomTrackIndex]);
    state.customPlaylistSortOrder = state.customPlaylistSortOrder === "desc" ? "asc" : "desc";
    sortSongsByAddedAt(items, state.customPlaylistSortOrder);
    if (currentKey) {
        const nextIndex = items.findIndex((song) => getSongKey(song) === currentKey);
        if (nextIndex >= 0) {
...
function clearLibraryDragState() {
    document.querySelectorAll(".playlist-item.dragging, .playlist-item.drag-over").forEach((item) => {
...
function handleLibraryDragStart(event) {
    const handle = event.target.closest(".playlist-item-drag");
    if (!handle) {
        return;
    }
    ...
    state.dragSort = {
        list: handle.dataset.dragList,
        index,
    };
    event.dataTransfer.effectAllowed = "move";
    event.dataTransfer.setData("text/plain", `${state.dragSort.list}:${index}`);
    handle.closest(".playlist-item")?.classList.add("dragging");
}

function handleLibraryDragOver(event) {
    const item = event.target.closest(".playlist-item[data-index]");
    const targetHandle = item?.querySelector(".playlist-item-drag");
    if (!item || !targetHandle || !state.dragSort || targetHandle.dataset.dragList !== state.dragSort.list) {
        return;
    }
    event.preventDefault();
    event.dataTransfer.dropEffect = "move";
    document.querySelectorAll(".playlist-item.drag-over").forEach((dragItem) => {
        if (dragItem !== item) {
...
function handleLibraryDragLeave(event) {
    event.target.closest(".playlist-item")?.classList.remove("drag-over");
}
...
function handleLibraryDrop(event) {
    ...
    event.preventDefault();
    const fromIndex = state.dragSort.index;
    const toIndex = Number(item.dataset.index);
    const items = getSortableLibraryItems(state.dragSort.list);
    if (!Number.isNaN(toIndex) && moveItemInArray(items, fromIndex, toIndex)) {
        persistLibrarySort(state.dragSort.list, fromIndex, toIndex);
    }
    clearLibraryDragState();
}

function initializeLibrarySearchHandlers() {
    if (dom.playlistSearchInput) {
        dom.playlistSearchInput.addEventListener("input", () => {
            state.playlistFilter = dom.playlistSearchInput.value;
            renderPlaylist();
        });
    }
    if (dom.favoritesSearchInput) {
        dom.favoritesSearchInput.addEventListener("input", () => {
            state.favoritesFilter = dom.favoritesSearchInput.value;
            renderFavorites();
        });
    }
    if (dom.customPlaylistSearchInput) {
        dom.customPlaylistSearchInput.addEventListener("input", () => {
            state.customPlaylistFilter = dom.customPlaylistSearchInput.value;
            renderCustomPlaylists();
        });
    }
}

function initializeLibraryDragSortHandlers() {
    [dom.playlistItems, dom.favoriteItems, dom.customPlaylistItems].filter(Boolean).forEach((container) => {
        container.addEventListener("dragstart", handleLibraryDragStart);
        container.addEventListener("dragover", handleLibraryDragOver);
        container.addEventListener("dragleave", handleLibraryDragLeave);
        container.addEventListener("drop", handleLibraryDrop);
        container.addEventListener("dragend", clearLibraryDragState);
    });
}
...
function renderPlaylist() {
    ...
    dom.playlist.classList.remove("empty");
    const entries = getFilteredSongEntries(state.playlistSongs, state.playlistFilter);
    if (entries.length === 0) {
        dom.playlistItems.innerHTML = '<div class="library-no-results">没有找到匹配歌曲</div>';
        updatePlaylistActionStates();
        return;
    }
    const playlistHtml = entries.map(({ song, index }) => {
        ...
        <div class="playlist-item" data-index="${index}" role="button" tabindex="0" aria-label="播放 ${song.name}" data-favorite-key="${songKey}">
            ...
            ${createDragHandle("playlist", index)}
        </div>`;
    }).join("");
...
function renameSelectedCustomPlaylist() {
    const playlist = getSelectedCustomPlaylist();
    if (!playlist) {
        showNotification("请先创建歌单", "warning");
        return;
    }
    ...
    saveCustomPlaylistState();
    renderCustomPlaylists();
    showNotification("歌单已重命名", "success");
}

function deleteSelectedCustomPlaylist() {
    const playlists = ensureCustomPlaylistsArray();
    const playlist = getSelectedCustomPlaylist();
    if (!playlist) {
        showNotification("请先创建歌单", "warning");
    ...
    if (!window.confirm(`删除歌单“${playlist.name}”？`)) {
        return;
    }
...
function renderCustomPlaylists() {
    ...
    [
        ...
        dom.sortCustomPlaylistBtn,
        dom.deleteCustomPlaylistBtn,
        dom.mobileRenameCustomPlaylistBtn,
        dom.mobileSortCustomPlaylistBtn,
        dom.mobileDeleteCustomPlaylistBtn,
    ].forEach((button) => {
        if (button) {
            button.disabled = playlists.length === 0;
            button.setAttribute("aria-disabled", playlists.length === 0 ? "true" : "false");
        }
    });
    updateSortButtonIcons([dom.sortCustomPlaylistBtn, dom.mobileSortCustomPlaylistBtn], state.customPlaylistSortOrder);
    ...
    state.customPlaylistSortOrder = inferAddedAtOrder(items);
    updateSortButtonIcons([dom.sortCustomPlaylistBtn, dom.mobileSortCustomPlaylistBtn], state.customPlaylistSortOrder);
    const entries = getFilteredSongEntries(items, state.customPlaylistFilter);
    [dom.sortCustomPlaylistBtn, dom.mobileSortCustomPlaylistBtn].forEach((button) => {
        if (button) {
            button.disabled = items.length === 0;
            button.setAttribute("aria-disabled", items.length > 0 ? "false" : "true");
        }
    });
    ...
    if (entries.length === 0) {
        dom.customPlaylistItems.innerHTML = '<div class="library-no-results">没有找到匹配歌曲</div>';
        return;
    }

    dom.customPlaylistItems.innerHTML = entries.map(({ song, index }) => {
        const artistValue = Array.isArray(song.artist)
            ? song.artist.join(", ")
            : (song.artist || "未知艺术家");
...
function removeFromPlaylist(index) {
    ...
    if (removingCurrent) {
        if (state.playlistSongs.length === 1) {
            ...
            dom.currentSongTitle.textContent = "未选择歌曲";
            updateMobileToolbarTitle();
...
function updateFavoriteActionStates() {
    ...
    [dom.sortFavoritesBtn, dom.mobileSortFavoritesBtn].forEach((button) => {
        if (button) {
            button.disabled = !hasFavorites;
            button.setAttribute("aria-disabled", hasFavorites ? "false" : "true");
        }
    });
    updateSortButtonIcons([dom.sortFavoritesBtn, dom.mobileSortFavoritesBtn], state.favoriteSortOrder);
    if (dom.addAllFavoritesBtn) {
...
function renderFavorites() {
    ...
    const entries = getFilteredSongEntries(favorites, state.favoritesFilter);
    if (entries.length === 0) {
        dom.favoriteItems.innerHTML = '<div class="library-no-results">没有找到匹配歌曲</div>';
        updateFavoriteIcons();
        updateFavoriteActionStates();
        return;
    }
    const favoritesHtml = entries.map(({ song, index }) => {
        const artistValue = Array.isArray(song.artist)
            ? song.artist.join(", ")
            : (song.artist || "未知艺术家");
        ...
        <div class="playlist-item${isCurrent ? " current" : ""}" data-index="${index}" role="button" tabindex="0" aria-label="播放 ${song.name}" data-favorite-key="${songKey}">
            ...
            ${createDragHandle("favorites", index)}
        </div>`;
    }).join("");
...
function handleImportedFavoriteItems(rawItems) {
    ...
    sanitizedSongs.forEach((song) => {
        ...
        assignAddedAt(song, importedAt - importedFavorites.length);
